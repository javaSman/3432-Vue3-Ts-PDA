<template>
  <van-field v-model="label" readonly :name="item.prop" :label="item.label" :placeholder="'请选择' + item.label"
    :rules="item.rules" :required="item.rules && item.rules.length > 0" @click="showPicker = true">
    <template #input>
      <van-dropdown-menu>
