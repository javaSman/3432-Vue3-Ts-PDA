...
function getBarcodes() {
  if (form.value.imBarcode) {
    ...
    WMSAPI.post(
      APIName,
      {
        allocationID: form.value.purchaseOrder,
        barcodes: [Barcode],
        BoxID: form.value.boxId,
        LocationID: form.value.locationId
      },
      'allocationorder/ReceiveBarCode'
    ).then((res) => {
      if (res.success) {
        form.value.message = res.message as string
        header.value.inWarehouseCode = res.header.inWarehouseCode as string
...
function handleConfirm() {
  formComponent.value?.refForm.validate().then(() => {
    ...
    WMSAPI.post(
      APIName,
      { allocationID: form.value.purchaseOrder, barcode: form.value.imBarcode },
      'allocationorder/ReceiveSubmit'
    ).then((res) => {
      if (res.success) {
